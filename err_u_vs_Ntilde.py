...
r = 1
R = 5
# x_axis = np.linspace(1.4, 2.8, 10) # = [1.4, 1.533..., ..., 2.8] 
Ntilde = [4, 5, 6, 8, 10, 12, 14, 17, 20]
...
for k in range(5):		# different runs to get more data for box plots
	if re_calculate:
		...
		data = [vg.GridScheme(d=1, r=r, R=R, M=int((n**4/R**4)*200), Ntilde=n, NbP=NbP, K_z=0.1, plot_iter=False) for n in Ntilde]
...
ax1.set_ylabel(fr"$\sup_x\ \log (\Delta u^{NbP}_{r})$")
...
ax2.set_ylabel(fr"$\sup_x\ \log (\Delta \bar u^{NbP}_{r})$")
