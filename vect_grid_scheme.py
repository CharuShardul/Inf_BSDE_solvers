...
class GridScheme:
    ...
    def interpol(self, x, gridx):
        ...
        :param x: numpy array of coordinates of x in R^d
        :param gridx: value of the function on the grid space converted to 1-d
    ...
    #@jit(nopython=True, fastmath=True)
    def sampleW_tx(self, x, t):
        # One sample for each x and t
        W_E = x[None, :, :] + np.sqrt(t)[:, :, None] * np.random.randn(self.M, self.grid_size, self.d)
        return W_E
    ...
    def phi(self, u_grid, ub_grid):
        ...
        W_E_grid = self.sampleW_tx(self.x_grid, E_grid)
        W_Eb_grid = self.sampleW_tx(self.x_grid, Eb_grid)

        # Next, we interpolate u and \bar{u} at the above samples of exponential times.
        exp_interp_grid_u = self.interpol(W_E_grid, u_grid)
        exp_interp_grid_ub = self.interpol(W_E_grid, ub_grid)
        ...
        gam_interp_grid_u = self.interpol(W_Eb_grid, u_grid)
        gam_interp_grid_ub = self.interpol(W_Eb_grid, ub_grid)
        ...
        phi_Pu = np.mean((self.f(W_E_grid, exp_interp_grid_u, exp_interp_grid_ub) + self.a * exp_interp_grid_u)
    ...
    def PicIter(self):
        if self.d == 1:
            ...
            if self.plot_iter:
                ...
                plt.show()
            # fig.savefig("Numerical_experiments/Grid_scheme/1d/u_and_ub_trdom3.png", bbox_inches='tight')
