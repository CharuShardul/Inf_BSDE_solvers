...
if re_calculate:
    for K_z in K_z_values:
        ...
        solver = NNPicardSolver(d=1, K_z=K_z)
        solver.main()
...
ax1.set_ylabel(r"$\log (\Delta u_{d}^{n})$, n=5, d=1")
...
ax2.set_ylabel(r"$\log (\Delta \bar{u}_{d}^{n})$, n=5, d=1")
...
#plt.show()
plt.savefig('plots_article/err_K_z_NNPic.pdf', dpi=300)
